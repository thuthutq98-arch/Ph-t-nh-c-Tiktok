...
// Save settings to localStorage (survives server restart + page reload)
function saveRoomToLocal() {
  try {
    ...
    // Always save to default key
    localStorage.setItem('room__default', data);
    // Also save to room-specific key if connected
    if (currentRoom) {
      localStorage.setItem('room_' + currentRoom, data);
    }
  } catch(e) {}
}
...
function loadRoomFromLocal() {
  try {
    // Try room-specific first, then default
    let raw = null;
    if (currentRoom) {
      raw = localStorage.getItem('room_' + currentRoom);
    }
    if (!raw) {
      raw = localStorage.getItem('room__default');
    }
...
async function loadConfig() {
  try {
    ...
    let serverCfg = await res.json();

    // Restore from localStorage if server lost data (room recreated after restart)
    const localCfg = loadRoomFromLocal();
    if (localCfg) {
      // Gift mappings: prefer local if server is empty
      if ((!serverCfg.giftMappings || Object.keys(serverCfg.giftMappings).length === 0) && localCfg.giftMappings && Object.keys(localCfg.giftMappings).length > 0) {
        serverCfg.giftMappings = localCfg.giftMappings;
      }
      // Playlist: prefer local if server is empty
      if ((!serverCfg.playlist || serverCfg.playlist.length === 0) && localCfg.playlist && localCfg.playlist.length > 0) {
        serverCfg.playlist = localCfg.playlist;
      }
      // Other settings: restore from local
      if (localCfg.volume !== undefined) serverCfg.volume = localCfg.volume;
      if (localCfg.minGiftCoins !== undefined) serverCfg.minGiftCoins = localCfg.minGiftCoins;
      if (localCfg.chatTtsEnabled !== undefined) serverCfg.chatTtsEnabled = localCfg.chatTtsEnabled;
      if (localCfg.ttsRate !== undefined) serverCfg.ttsRate = localCfg.ttsRate;
      if (localCfg.ttsPitch !== undefined) serverCfg.ttsPitch = localCfg.ttsPitch;
    }

    systemConfig = { ...systemConfig, ...serverCfg };
    ...
    // Sync restored config back to server
    if (localCfg && currentRoom) {
